...
export function logActivity(type: 'camera' | 'screen' | 'system', description: string, metadata?: any) {
...
export function getRecentActivitySummary(): string {
  ...
  return logs.map(l => `[${new Date(l.timestamp).toLocaleTimeString()}] ${l.type.toUpperCase()}: ${l.description}`).join('\n');
}
